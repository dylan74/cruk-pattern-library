<?php
  ...
  function listFilesInFolder($dir) {
    ...
    // echo '<ul>'; // As opened in sidebar.php
    foreach ($files as $file) {
      $dir = 'patterns'; // Overwrite dir value passed from sidebar.php that was to overcome the different directories
      $path = $dir.'/'.$file;
      if (is_dir($path)) {
  ...
  function renderTitleFromPath($path, $parent) {
    ...
    if ($parent) {
      ...
      // echo '<a href='.$home_url.'"sg-'.$id.'">'.$filename.'</a>';
